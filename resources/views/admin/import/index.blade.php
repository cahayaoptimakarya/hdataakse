            ...
            <div class="card-body py-8 px-10">
                <form id="import_shipments_form" action="{{ route('admin.import.shipments') }}" method="POST" enctype="multipart/form-data" class="form">
                    ...
                    <div class="mb-8">
                        ...
                        <div class="text-muted small mt-2">Header wajib memuat kolom resi, sku, dan jumlah.</div>
                    </div>
                    <div class="d-flex align-items-center gap-3">
                                ...
                                <div class="table-responsive mh-300px">
                                    <table class="table table-row-dashed align-middle mb-0">
                                        ...
                                        <tbody id="ship_skipped_list">
                                            <tr><td colspan="3" class="text-muted text-center">Tidak ada data dilewati.</td></tr>
                                        </tbody>
                                    </table>
                                </div>
                            </div>
    ...
    <div class="col-xl-4">
        <div class="card h-100">
            ...
            <div class="card-body py-6 px-8">
                ...
                <p class="text-muted small mb-3">Contoh sederhana:</p>
                <pre class="rounded bg-light p-4 mb-0">resi,sku,jumlah
ABC123456789,SKU-01,3
ABC123456789,SKU-02,1
XYZ987654321,SKU-05,2</pre>
            </div>
        </div>
    </div>
</div>
    ...
    function updateShipSummary(summary) {
        ...
        if (duplicates.length === 0) {
            ...
            const items = duplicates.map(d => `
                <div class="d-flex align-items-start mb-3">
                    ...
                    <div class="flex-grow-1">
                        <div class="fw-bold">${d.resi_number} &ndash; ${d.sku}</div>
                        <div class="text-muted small">${d.reason || 'Duplikat'}</div>
                    </div>
                </div>
            `);
        ...
        if (shipSkippedList) {
            ...
            if (skippedEntries.length === 0) {
                shipSkippedList.innerHTML = '<tr><td colspan="3" class="text-muted text-center">Tidak ada data dilewati.</td></tr>';
            } else {
